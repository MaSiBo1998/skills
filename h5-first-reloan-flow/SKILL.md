...
本 skill 只负责首贷/复贷状态流和订单详情开发，是独立于具体项目的通用工作流。首复贷与进件是两个独立场景：进件负责 Apply 步骤页、Entry、表单草稿、拍照/联系人和 app-specific 进件差异；首复贷负责首页状态、产品列表、产品详情、借款确认、放款、失败、还款、首复贷 banner 和相关原生/风控闭环。
...
- 不属于本 skill：Apply 步骤页、进件 Entry、工作/联系人/个人/证件/人脸/银行卡步骤、进件 app-specific 差异。首复贷状态中出现“未授信/继续进件/编辑资料”时，本 skill 只负责状态展示和入口跳转，不实现 Apply 表单步骤；首复贷还款页、支付页等本场景页面一旦包含真实输入框，写入 `constraint_areas=["form-input"]` 并在本业务页面内完成适配。
