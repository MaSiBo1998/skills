...
- 用户已提供准确数据结构和类型、接口文档或现有类型已明确结构时，必须检查代码按固定结构直接取值或解析，未新增多层字段探测、旧字段兼容、复杂 helper 或本地文案兜底；只有真实数据证明会崩溃时，才允许最小错误隔离。
