...
- **检查项**:
  ...
  - 如本次涉及首复贷 banner，接口 URL 已收敛到 `services/api/config` 等项目 API 配置层；多个 banner 以 3 秒间隔轮播而非同时展示；首页/App 列表、还款页、首贷拒绝页等展示位符合需求；空数组不占位，接口失败静默降级；外链、App 内跳和不跳转三类点击行为均走统一 bridge/工具函数并已验证
  - 还款方式选择后的支付接口返回处理完整：外链打开、非 URL 凭证进入支付过渡页、Nequi 空字符串仍进入支付过渡页并展示打开钱包 App 确认提示、PSE 空链接按异常提示、渠道识别、银行选择后继续支付和兜底提示均可用
