...
- **预算**:
  - 非视觉改动、接口字段改动、逻辑顺序调整、文案改动和可由静态搜索证明的小改：默认不截图。
  - `quick`：默认 0 轮截图；只在用户明确要求或出现阻塞级可见风险时才启动目标页面截图。
  - `focused`：最多 1 轮截图，只覆盖受影响页面和必要视口。
  - `full`：默认最多 1 轮截图；设计图复原、图片压缩、复杂布局重构、首屏展示风险或用户明确要求时最多 2 轮。
...
- **注意**: `quick` 不默认执行构建；`focused` 只有触及样式入口、全局适配、资源加载、构建配置、依赖或组件导入导出的打包风险、用户要求或发布前时执行构建；不能因为缺少独立 type-check 命令而自动用 build 兜底；仅命中 Apply、首复贷、管理后台等业务路径或文件名，不构成执行构建的理由；`full/release` 必须执行构建
