...
- 实现前只记录暂定验收等级，最终等级必须在 diff 形成后按实际影响范围确认；业务场景、接口关键词、参考截图和弹窗形态本身都不能单独触发 `full`。
- `full` 表示覆盖完整静态与业务专项检查，不等于必须启动浏览器或截图。浏览器属于独立升级项，只在用户明确要求，或存在静态检查无法覆盖的运行态阻塞风险时执行。
- 单个现有页面内新增或调整 Popup、Modal、Dialog、局部 overlay，即使视觉上接近全屏，也不按大型布局改造处理；不得仅为证明单个弹窗样式而创建 mock API、临时测试页、Playwright fixture 或截图脚本。
- 同一证据不重复执行：若 `build` 已包含 `tsc`，不再单独跑 type-check；全量 lint 仅被本次 diff 外的存量错误阻断时，记录基线错误后只跑一次目标文件 lint，不重跑全量 lint。
- 完成“一组完整证据”后立即停止。一组完整证据是当前等级要求的 diff 审查、必要静态命令，以及风险明确要求的 build；命令已通过或已定位为无关基线后，不得为了增加确定感继续追加 dev server、浏览器、mock、截图或同类命令。
