...
**场景 H（工作流自我更新）**: 必须检查 `spec-driven-development` 是否产出轻量规格、`workflow-orchestration-patterns` 是否完成编排审查、`llm-evaluation` 是否执行至少 6 类回归样例；同时确认修改过的 skill 通过 `quick_validate.py`、运行时目录已同步、失败样例已修复或进入待确认沉淀项。
