...
截图是高成本证据，只在需要证明视觉、布局、资源或交互展示风险时使用，不作为每次开发完成后的默认动作。

- 非视觉改动、接口字段改动、逻辑顺序调整、文案改动和纯静态搜索可证明的小改，不启动 dev server，不截图。
- `quick`：默认 0 轮截图；若用户提供参考图或本次是可见 CSS 微调，最多 1 张目标页面/目标视口截图，不做截图后循环微调。
- `focused`：最多 1 轮截图，只覆盖受影响页面和必要视口；截图发现问题后优先用代码 diff、样式计算或静态搜索确认修复，不默认再截一轮。
- `full`：默认最多 1 轮截图；只有设计图复原、图片压缩、复杂布局重构、首屏展示风险或用户明确要求时，最多升级到 2 轮。
- 设计图复原：最多 2 轮截图，第一轮确认整体还原，第二轮只验证第一轮发现的阻塞级视觉问题；第二轮后仍有细微差异，记录差异和人工设计走查项，不继续自动迭代。
- `release`：不因发版检查默认截图；优先检查 release-env、vConsole、构建产物和风险清单。只有发布本身涉及可见页面风险时，才按对应等级预算截图。
- 每次截图前先说明本轮要验证的风险点，例如首屏裁切、按钮溢出、图片失真、弹层层级、滚动容器或关键状态展示。没有明确风险点时跳过截图。
...
- **精确结构优先**：用户已提供准确数据结构和类型、KB contract 或现有类型已明确结构时，代码应按固定结构直接取值或解析；不要新增多层字段探测、旧字段兼容、复杂 helper 或本地文案兜底。接口返回格式已确定时只读约定字段，例如错误提示只返回 `msg` 时不得额外兜底读取 `message`、旧字段或本地业务文案。
...
- **精确结构优先**：用户已提供准确结构/类型时，按固定结构取值或解析，避免不必要兜底；接口返回格式已确定时只读约定字段，不额外兜底旧字段或本地业务文案。
