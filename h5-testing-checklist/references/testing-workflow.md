...
- 发布前必须使用 `release`。
- 任意 App 内嵌 H5、App WebView 入口、官网域名挂载给 App 打开的 H5，即使只是 `focused` 小改，也必须执行 WebView 兼容专项检查；不能只在用户明确说“低版本手机”或“Flutter WebView”时才检查。
