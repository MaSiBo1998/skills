...
- “new-h5 复制 prestaone-h5，只替换接口地址和混淆字段”：场景 B，调用 `h5-api-mapping` 的同结构混淆字段替换，不改业务流程。
- “首复贷未确认页补一个按钮，不换接口文档”：场景 C，调用 `h5-first-reloan-flow`，不调用 API mapping。
- “进件新增键盘遮挡处理，不换字段”：场景 D，调用 `h5-apply-flow`，不调用 API mapping。
- “危地马拉进件换新 swagger，业务流程不变”：场景 D 业务归属 + 先调用 `h5-api-mapping` 同结构替换，再执行 country profile 与进件专项验收。
- “设计图还原页面”：场景 F，先调用 `design-image-analysis`，再调用 `design-image-restore`。
...
不同场景只收集本次执行真正需要的信息，缺少必填项时再询问用户；可选项缺失不得阻断流程。
...
- 交付完成后检查目标项目 `release-env`：
  - 若存在且首个非空值为 `mx / co / ng`，在交付末尾询问是否进入场景 G 执行发布。
  - 用户确认发布后，才调用 `h5-release-tag`。
  - 若 `release-env` 缺失、为空或非法，普通交付不阻断；仅记录“当前不可直接发布”，用户要求发布时再提示修正或手动指定国家。
  - `h5-release-tag` 不判断业务是否应该发布，只负责被调用后的构建、提交、打 Tag 和推送。
