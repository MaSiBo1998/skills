...
- 纯机械小改只有在用户已给出并确认绝对项目路径、或当前工作目录已经唯一确定目标仓库，且需求不含项目名/App 名/官网/协议等定位语义时，才可标记 `知识库=跳过`；不能用“小改”跳过项目路径索引。
...
这是实现前硬门禁，不受 quick/focused 或快速通道影响。定位结果必须记录为 `project_resolution`，至少包含用户说法、页面/项目用途、KB 命中路径、候选路径和最终选择依据；未完成时不得进入目标项目编辑。

同一 App 下存在官网、公司官网、App 内嵌或投放等多个相似目录时，必须按 KB 的“用途”列精确匹配用户说法：用户只说“官网/协议”时选择标记为“官网/协议项目”的仓库；只有用户明确说“公司官网”或给出公司主体时，才选择 `*-company` 等公司官网仓库。不得根据 `-company`、`-home` 或目录新旧感猜测用途。
