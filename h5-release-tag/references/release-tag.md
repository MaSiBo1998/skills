...
用于独立执行“发布代码 / 发版 / 打 tag”流程。基于项目根目录 `release-env` 自动识别国家，完成构建校验、提交、打标与推送。本流程只在用户直接要求发布，或 `front-workflow` 交付出口确认进入场景 G 后执行；不负责判断普通业务改动是否应该发布。
