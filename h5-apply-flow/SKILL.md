---
name: h5-apply-flow
description: H5 进件申请流程开发。用于新增或修改 Apply 页面、路由、步骤顺序、Entry 逻辑、表单草稿、原生桥接、拍照/联系人子流程、输入框聚焦滚动防键盘遮挡，以及按 appName 的 KB 文档和目标项目代码适配进件差异。不要用于首贷/复贷状态流，首复贷使用 h5-first-reloan-flow。
---

# H5 进件流程

本 skill 只负责 Apply 进件流程。不同 app 的功能差异来自 `Work/API/apps/<appName>` 的 app 文档、API contract、原生交互文档和目标项目代码；不再按国家维护功能分叉规则。首贷/复贷状态流是独立场景，归属 `h5-first-reloan-flow`。

## 维护边界

- `SKILL.md` 只保留进件场景入口、跨项目强约束和高频防错规则。
- 旧/新流程判断写入 `references/flow-variants.md`。
- 通用进件步骤、表单草稿、自动弹窗和公共约束触发点写入 `references/apply-flow.md`。
- 原生桥接协议写入 `references/native-methods.md`。
- app-specific 字段、配置、原生混淆和功能差异读取 `Work/API/apps/<appName>` 与目标项目代码，不在 workflow 中维护独立功能差异文件。
- 后续沉淀若只是单个项目字段、组件名或接口名，默认不写入本 skill；只有形成跨项目判断标准时才进入对应 reference。

## 执行方式

1. 确认产品、appName、项目根目录和本次需求类型，并自动判断是否启用 vendor 架构；vendor 默认为不执行，只有用户明确要求、checkpoint 已启用或项目现有约束需要时才启用。
2. 加载 `references/flow-variants.md`，先判断旧流程还是新流程；参考项目不可用时按抽象合同和目标项目证据执行，不阻断。
3. 加载 `references/apply-flow.md`，按其中原流程执行。
4. 读取个人知识库的 H5 场景知识：通用进件读 `Work/H5/业务场景/进件流程.md`；涉及表单输入、键盘或移动端交互时读 `Work/H5/公共规范/移动端表单与交互约束.md`；涉及 App WebView 时读 `Work/H5/公共规范/App WebView兼容.md`；涉及视觉还原、截图预算、拍照图片质量时读 `Work/H5/公共规范/视觉还原与截图预算.md`。
5. 只有本次涉及接口 contract、新字段、新接口地址、新项目迁移、字段替换、app-specific 原生协议或配置差异时，先用 `api-kb-contract-reader` 读取 `Work/API/apps/<appName>` contract/app 文档；KB 缺失时先用 `api-doc-kb-archiver` 入库；需要 H5 代码落地时再交给 `h5-api-mapping`。普通进件页面/交互补充复用现有 API。
...
- 涉及原生交互时必须遵守 `references/native-methods.md` 的统一桥接协议；只要有原生方法交互，就判定为 App 内嵌 H5，并在 checkpoint 写入 `constraint_areas=["webview"]`，若同时存在真实输入项再追加 `form-input`。
...
- 不按国家套用功能差异；所有 app-specific 字段、配置、原生混淆、步骤配置和接口结构以 `Work/API/apps/<appName>`、用户确认材料和目标项目代码为准。
- 首贷/复贷、订单状态、产品详情、未确认、放款、还款和 App 列表不属于本 skill；遇到这些任务应切换到 `h5-first-reloan-flow`。
- Apply 页面包含真实输入框、固定底部操作区、弹层选择器、输入清洗、粘贴或提交兜底时，写入 `constraint_areas=["form-input"]`；具体键盘避挡、选择器 blur、16px 字号和滚动容器验收由 `h5-testing-checklist` 与 `Work/H5/公共规范/移动端表单与交互约束.md` 承接。
...
- Apply 页面涉及点击高亮、focus 线框、滚动容器、设计图还原或布局溢出时，写入 `constraint_areas=["visual-layout"]`；拍照按钮等业务关键按钮仍需作为进件触发点验收，但公共视觉规则由区域清单承接。
