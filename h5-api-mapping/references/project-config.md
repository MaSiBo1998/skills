...
工作流中的可选配置收集模块。每次执行场景 B/C/D，或管理后台场景 I 中涉及接口字段替换时，先从目标项目代码、环境变量、接口文档和用户已给材料中提取配置；仅当缺失配置会阻断接口联调、加密、登录态或发布判断时，才询问具体缺失项。

---

## 可选项目配置

以下配置项由用户按需提供，也可以由工作流从项目代码和接口文档中提取。如果没有提供且不阻断当前开发，则使用项目代码中的现有值并在交付中标记待验。
