# 场景 E — 官网需求、协议展示与挂载小 H5

用于官网项目相关需求：官网协议展示、隐私入口、协议文档静态 HTML、App 内嵌官网协议链接问答，以及后续挂载在官网域名下的独立小 H5。
...
也用于 App 内嵌协议/隐私相关开发问答：例如 App WebView 加载哪类协议链接、H5 如何承接原生入口、官网隐私入口与 App 内链接如何对齐、协议展示验收口径如何判断。该场景的固定实现规则待后续需求确认后再沉淀。
...
- **App 内嵌开发问答**：用户询问 App 内嵌协议、隐私入口、WebView 链接、H5 与原生跳转配合、开发/验收口径时，先读取现有项目路由、协议链接、原生桥接或入口配置；在需求未沉淀前，只输出基于当前项目事实的判断和待确认项，不固化新规则。
...
适用于协议/隐私相关的 App 内嵌开发问题，先作为场景入口保留，等用户补充稳定需求后再完善为固定流程。
