---
name: h5-embedded-app-workflow
description: H5 内嵌 app 项目全流程工作流。支持新项目开发、架构改造、进件功能开发。自动完成设计图分析、接口文档解析、vendor 架构建立、Vite external 配置与全链路测试验收。
---

# H5 Embedded App Workflow

这是一个面向 Claude Code 的自动化开发工作流 Skill，处理三类场景：

| 场景 | 说明 |
|------|------|
| **A — 新app首复贷功能实现** | 新项目开发 + vendor 架构 + Figma 还原 + 接口适配 |
| **B — 架构改造** | 不改业务逻辑，只改构建配置，建立 static-app/vendor 本地加载架构 |
| **C — 进件功能开发** | 新增或修改进件申请流程 + Figma 还原 + 接口适配 + vendor 架构，只改 Apply 相关页面 |

---

## 使用方式

在项目目录中执行以下步骤：

**Step 1.** 触发本 Skill（说"用 h5 工作流"或类似语句）

**Step 2.** 选择场景：Claude 会列出 A/B/C 让你选

**Step 3.** Claude 读取对应场景的详细流程文件并执行

---

## 什么时候触发

- 需要根据 Figma 设计稿进行像素级还原
- 需要接入新接口文档
- 需要建立 static-app/vendor 静态资源本地加载架构
- 需要新增或重构进件申请流程
- 需要在完成开发后自动测试验收

## 不适用场景

- 从零新建 H5 项目脚手架
- 纯后端开发、数据库设计、运维部署
- 单纯生成 Git 分支名、发布 Tag、处理飞书 Bug

---

## 全局强约束

- 构建产物必须使用自定义协议 `local-resource://h5/`（或 `APP_RESOURCE_PREFIX` 配置），不可使用 `file://` 或 CDN 路径
- 构建产物中不得包含框架代码（必须通过 externals 排除）
- `static-app/` 目录必须与 `src/` 同级，不在 `public/` 目录内
- 所有框架库必须保留在 `devDependencies` 中（JSX 运行时子模块需从 node_modules 解析）
- `external` 列表必须精确列出主模块，不可用 `^react(\/.*)?$/` 全覆盖
- 执行中发现 Skill 自身疏漏或用户提出优化建议，必须同步更新本 Skill 文件

---

## 成功标准

- 正确识别并执行所选场景
- 成功建立 static-app/ 基线依赖架构（如果场景包含）
- 构建产物使用自定义协议，不含框架代码
- 15 项测试 CheckList 逐项执行并输出结果
- 交付清晰的测试和验收说明
- Skill 改进建议输出并同步更新

---

## 推荐触发语句

- 用 h5 工作流帮我做这个需求
- 使用 h5-embedded-app-workflow
- 跑一下 h5 工作流
