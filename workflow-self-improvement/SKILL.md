...
- 引用扫描失真：把占位符、命令片段、斜杠分隔的概念标签/枚举/比例/尺寸、未解析或被裁剪的 CODEX_HOME 记忆路径片段、文件发现 glob、目标项目示例文件、已发现辅助 skill 的说明性导航，或外部/辅助 skill 文档中的跨 skill 导航引用误当成本仓库缺失引用，或扫描脚本报错但仍把污染输出当成有效结论。
