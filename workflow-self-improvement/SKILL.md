...
description: 工作流自我更新成长。用于用户要求“记住、记一下、下次按这个来、下次自动、沉淀一下、规则沉淀、完善工作流、巡检工作流、更新 skill、自我成长、规则不对、自动沉淀、自动学习、补回归、同步运行时、确认沉淀”时，或执行中发现可复用经验、遗漏验收、重复人工修正、新 app 文档规则、新接口模式、发布规则变化时，先判断沉淀归属并输出沉淀提案，用户确认后再修改主工作流或对应子 skill、校验并同步运行时。
---
...
1. 发现可沉淀项：
   ...
   - 实际执行中出现重复判断、重复人工修正、遗漏验收、app-specific 文档规则或接口模式无法被现有 skill 覆盖。
   - 子 skill 输出与项目真实约束冲突。
...
| 项目特例 | 只在单个产品、app、接口或页面成立的事实 | 默认不沉淀；必要时写到对应 KB app 文档或项目 reference |
...
| Apply、Entry、步骤页、原生交互、app-specific 文档规则 | `h5-apply-flow` + `Work/API/apps/<appName>` |
...
- 不自动创建新的按国家分叉的进件 skill，也不新增功能差异文件；app-specific 字段、配置、原生协议和接口结构优先沉淀到 `Work/API/apps/<appName>`，workflow 只保留读取和调度规则。
